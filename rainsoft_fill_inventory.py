...
class rainsoft_fill_inventory(osv.osv):
    _name='stock.fill.inventory'
    _inherit='stock.fill.inventory'
    
    def _get_racks(self,cr,uid,context=None):
	res=[]
	ids = self.pool.get('rainsoft.rack').search(cr,uid,[],context=context)
	racks = self.pool.get('rainsoft.rack').read(cr,uid,ids,['name'],context=context)
	for rack in racks:
	    res.append((rack["id"],rack["name"]))
	return res

    _columns={
        'loc_rack':fields.selection(_get_racks,"Rack")
    }
    ...
    def fill_inventory(self, cr, uid, ids, context=None):
        ...
        for location in location_ids:
            ...
            for move in move_obj.browse(cr, uid, move_ids, context=context):
                ...
                if fill_inventory.loc_rack and int(product.loc_rack) != int(fill_inventory.loc_rack):
		   continue      
